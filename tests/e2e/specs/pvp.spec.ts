...
import {
  ...
  DEFAULT_TOURNAMENT_ROSTER_SIZE,
  type TestAccount,
} from '../fixtures';

/** 赛事默认 rosterSize（config 未指定时为 3）。 */
const DEFAULT_ROSTER_SIZE = DEFAULT_TOURNAMENT_ROSTER_SIZE;
...
test.describe('PVP 锦标赛', () => {
  ...
  test('8 人满编开赛并决出冠军，冠军可领取奖池', async ({ page, request }) => {
    ...
    const main = await setupTournamentPlayer(request, 'pvp');
    ...
    for (let i = 0; i < 7; i += 1) {
      const Brain = await setupTournamentPlayer(request, 'pvp');
