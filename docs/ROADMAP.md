...
> **M4.3 实测记录（2026-09-04，本地 MariaDB + 进程直跑）**：PVP 调度器已接入确定性 bracket/bye、4 人最低人数取消退款、8/16 人逐轮自动模拟、报名快照重放、ContestRecord 幂等、详情/bracket/管理员 start API、参赛双方战报权限和 Web 结果展示。锁后时间、PVP 锁顺序、管理员审计与 outsider 脱敏均有测试覆盖；全量质量门 35 个测试文件、283 个测试通过，workspace typecheck/lint/build 通过，`20260903000000_pvp_matches` 已应用到开发库。Docker、浏览器和视觉验收按约定延后到最终部署窗口。

> **M4.4 实测记录（2026-09-04，本地 MariaDB + 进程直跑）**：PVP 调度器已将赛事配置中的 `rules.qualityScoring` 透传至每场 `DuelInput`，兼容顶层 camelCase 与 snake_case 写法；PVP tiebreak 固定为 `SUDDEN_DEATH`，不允许赛事配置改写。新增质量计分端到端断言；聚焦调度器/duel 测试 20/20 通过，全量质量门 35 个测试文件、284 个测试通过，workspace typecheck/lint/build 与 `git diff --check` 均通过。Docker、浏览器和视觉验收按约定延后到最终部署窗口。

> **M4.5 实测记录（2026-09-04，本地 MariaDB + 进程直跑）**：PVP 调度器已在对手未解出携带预制题时，依据 `economy.yaml → pvp` 配置向出题方写入 `PVP_PROBLEM` 声誉日志；稳定审计键保证同题同场至多计次，单题单届收益封顶 12 点，且结算与比赛状态更新同事务幂等。新增未解题收益/重放和单题单届上限集成测试；聚焦 scheduler+duel 21/21 通过，全量质量门 35 个测试文件、286 个测试通过，workspace typecheck/lint/build、Prisma validate/migrate status 与 `git diff --check` 均通过。Docker、浏览器和视觉验收按约定延后到最终部署窗口。

> **M4.6 实测记录（2026-09-04，本地 MariaDB + 进程直跑）**：PVP 结束时生成赛事/用户唯一 `PvpRewardGrant`，按冠军、亚军、四强及首轮胜者发放默认或管理员配置奖池；冠军强制获得且只能获得 1 张 `tag-card`。新增截止前奖池 PATCH、公开奖励查询、本人领取接口，领取事务原子发放钱/道具/声誉并以 `claimedAt` 幂等；Web 管理端可填写奖池 JSON，PVP 页展示公示及领取状态。新增默认奖池、管理员冻结、领取重放测试；全量质量门 35 个测试文件、289 个测试通过，workspace typecheck/lint/build、Prisma validate/migrate status 与 `git diff --check` 均通过。开发库迁移 `20260904120000_pvp_reward_grants` 已应用，Docker、浏览器和视觉验收按约定延后到最终部署窗口。
...
| [x] T4.3 | 锦标赛调度器：对阵树生成、批量排队模拟、轮次推进、结果公示页 | 16 人赛全自动跑完 |
| [x] T4.4 | 加赛（sudden_death）与平局分流落地 | 单测覆盖 |
| [x] T4.5 | 己方题库入赛：提交 N 题入赛场题池、对手未解出的声誉收益与每场计次上限 | 防刷规则生效 |
| [x] T4.6 | 奖励发放：tag-card 冠军独占+管理员配置奖池；公示领奖 | 发放幂等（重跑不重复发） |
